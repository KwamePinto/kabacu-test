...
router.get('/', authenticateAdminUser, async (req, res) => {
  try {
    // Case 1 (pre-fix): timed out → was treated as fail → wallet refunded → may need deduction
    const oldFlagged = await Transaction.find({
    ...
    // Case 2 (post-fix): timed out → saved as pending → wallet not refunded → needs verification
    const newFlagged = await Transaction.find({
      paymentMethod: 'wallet',
      status: 'pending',
    ...
    const all = [...oldFlagged, ...newFlagged]
    ...
    const notifications = all.map(tx => {
      ...
      return {
        ...
        icon:  'alert-triangle',
        color: isOld ? 'bg-danger' : 'bg-warning',
        time:  tx.createdAt,
